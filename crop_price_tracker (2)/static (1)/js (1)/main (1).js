...
const translations = {
    'en': { // English
        ...
        noResultsFound: 'No price data found for your selection. Please try different criteria.',
    ...
    'hi': { // Hindi
        ...
        noResultsFound: 'आपके चयन के लिए कोई मूल्य डेटा नहीं मिला। कृपया अलग मापदंड आजमाएं।',
    ...
    'ta': { // Tamil
        ...
        noResultsFound: 'உங்கள் தேர்வுக்கு விலை தரவு எதுவும் இல்லை. வேறு அளவுகோல்களை முயற்சிக்கவும்.',
        ...
        trendDescription: 'காலப்போக்கில் விலைகள் எவ்வாறு மாறியுள்ளன என்பதைப் பார்க்கவும்',
        nearbyMarkets: 'அருகிலுள்ள சந்தை விலைகள்',
        compareRegion: 'உங்கள் பகுதியில் உள்ள வெவ்வேறு சந்தைகளில் விலைகளை ஒப்பிடுங்கள்',
    ...
    'te': { // Telugu
        ...
        noResultsFound: 'మీ ఎంపిక కోసం ధర డేటా కనుగొనబడలేదు. దయచేసి వేరే ప్రమాణాలను ప్రయత్నించండి.',
...
function updateUILanguage() {
    // Update form labels
    document.querySelector('label[for="state"]').textContent = translations[currentLanguage].selectState;
    document.querySelector('label[for="district"]').textContent = translations[currentLanguage].selectDistrict;
    document.querySelector('label[for="market"]').textContent = translations[currentLanguage].selectMarket;
    document.querySelector('label[for="commodity"]').textContent = translations[currentLanguage].selectCommodity;
    document.querySelector('label[for="variety"]').textContent = translations[currentLanguage].selectVariety;
    document.querySelector('label[for="grade"]').textContent = translations[currentLanguage].selectGrade;
    document.querySelector('label[for="arrival_date"]').textContent = translations[currentLanguage].date;
    
    // Update button text
    document.querySelector('#priceForm button[type="submit"] span').textContent = translations[currentLanguage].getPrices;
    
    // Update loading text
    document.querySelector('#loading p').textContent = translations[currentLanguage].loadingData;
    ...
    if (minPriceLabel) minPriceLabel.textContent = translations[currentLanguage].minPrice;
    if (modalPriceLabel) modalPriceLabel.textContent = translations[currentLanguage].modalPrice;
    if (maxPriceLabel) maxPriceLabel.textContent = translations[currentLanguage].maxPrice;
    ...
    if (trendHeader) trendHeader.textContent = translations[currentLanguage].priceTrends;
    if (trendDesc) trendDesc.textContent = translations[currentLanguage].trendDescription;
    ...
    if (nearbyHeader) nearbyHeader.textContent = translations[currentLanguage].nearbyMarkets;
    if (nearbyDesc) nearbyDesc.textContent = translations[currentLanguage].compareRegion;
    if (noNearbyText) noNearbyText.textContent = translations[currentLanguage].noNearbyMarkets;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('priceForm').addEventListener('submit', function(event) {
        ...
        .then(data => {
            ...
            if (data.records && data.records.length > 0) {
                ...
                const nearbyMarketsContainer = document.getElementById('nearby-markets-table-container');
                const noNearbyMarkets = document.getElementById('no-nearby-markets');
                ...
                if (nearbyMarketsContainer && noNearbyMarkets) {
                    if (data.nearby_markets && data.nearby_markets.length > 0) {
                        displayNearbyMarkets(data.nearby_markets, data.records[0].Modal_Price);
        ...
        .catch(error => {
            ...
            if (noResultsDiv && noResultsMessage) {
                ...
                noResultsMessage.textContent = 
                    `${translations[currentLanguage].errorFetching}: ${error.message}. ${translations[currentLanguage].tryAgainLater}`;
            }
    ...
    function displayPriceData(records) {
        ...
        document.getElementById('min-price').textContent = '₹' + latestRecord.Min_Price;
        document.getElementById('modal-price').textContent = '₹' + latestRecord.Modal_Price;
        document.getElementById('max-price').textContent = '₹' + latestRecord.Max_Price;
    }
    ...
    function displayNearbyMarkets(markets, currentModalPrice) {
        ...
        try {
            ...
            markets.forEach(market => {
                ...
                const priceDiffClass = priceDiff >= 0 ? 'price-difference-positive' : 'price-difference-negative';
                ...
                row.innerHTML = `
                    ...
                    <td class="${priceDiffClass}">${priceDiffSign}₹${Math.abs(priceDiff || 0)}</td>
                `;
    ...
    function displayPriceChart(records) {
        try {
            ...
            const minPrices = records.map(record => parseFloat(record.Min_Price) || 0);
            const modalPrices = records.map(record => parseFloat(record.Modal_Price) || 0);
            const maxPrices = records.map(record => parseFloat(record.Max_Price) || 0);
    ...
    function fetchMarkets(state, district) {
        fetch(`/markets?state=${encodeURIComponent(state)}&district=${encodeURIComponent(district)}`)
        .then(response => response.json())
        .then(data => {
